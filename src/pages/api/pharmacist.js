...
export default function usePharmacist() {
    ...
    const addPharmacist = async (data) => {
        let { email, password, nik, firstName, lastName, phoneNum, dob, role, sipa } = data;
        dob = convertToTimestampString(dob);
        try {
             return await api.post("/api/v1/pharmacists/", { email, password, nik, firstName, lastName, phoneNum, dob, role, sipa })
        } catch (error) {
